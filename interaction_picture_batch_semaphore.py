...
parser.add_argument('--spin_state_command', type=str, default='', help='qutip command for defining the initial spin state, example command: "dd = qutip.tensor([(1/np.sqrt(2))*(u-1j*d),]*Natoms); spin_state = \'y\'"')
		...
		while len(processes) == max_processes:
			for i in range(len(processes)):
				...
				if p.poll() is not None:
					...
					del start_times[i]
					if p.poll() !=0:
						print(f"Std Err: {stderr.decode('ascii')}")
